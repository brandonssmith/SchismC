/*
 * Create a working Windows PE executable
 * This creates a minimal but functional PE file
 */
...
int main() {
    FILE *file = fopen("working_pe.exe", "wb");
    if (!file) {
    ...
    /* .text section data - simple x64 program that exits */
    U8 text_data[512] = {0};
    /* x64: mov rax, 0; ret */
    text_data[0] = 0x48; text_data[1] = 0xC7; text_data[2] = 0xC0; text_data[3] = 0x00; text_data[4] = 0x00; text_data[5] = 0x00; text_data[6] = 0x00; /* mov rax, 0 */
    text_data[7] = 0xC3; /* ret */
    
    fwrite(text_data, 1, 512, file);
    
    fclose(file);
    
    printf("Working PE executable created: working_pe.exe\n");
    return 0;
}
